#!/usr/bin/env bash
# The check sequence, run INSIDE the pinned Node image by scripts/check.sh.
...
step "keys (regenerate, compare committed hashes; k and rows)" scripts/keys.sh verify all
step "format" npm run format:check
step "lint (type-aware)" npm run lint
step "typecheck" npm run typecheck
step "build" npm run build
step "tests" npm test
step "separate project (notice-board from the packed library)" scripts/check-external-consumer.sh
